const { pool } = require('./src/config/db');
const crypto = require('crypto');

async function debugRegistration() {
    try {
        console.log('--- Starting Debug Registration V3 ---');
        const userRes = await pool.query('SELECT id FROM users LIMIT 1');
        const owner_id = userRes.rows[0].id;

        // Use valid UUID
        const device_uid = crypto.randomUUID();

        console.log('Attempting Device Insert with Valid UUID + MANUFACTURER...');
        try {
            const res = await pool.query(
                `INSERT INTO devices (owner_id, device_type, brand, model, purchase_year, serial_number, device_uid, device_uid_origin, current_state) 
                VALUES ($1, $2, $3, $4, $5, $6, $7, 'MANUFACTURER', 'ACTIVE') 
                RETURNING *`,
                [owner_id, 'Smartphone', 'DebugBrand', 'DebugModel', 2024, 'SN-UUID', device_uid]
            );
            console.log('✅ Success! ID:', res.rows[0].id);

            // Cleanup
            await pool.query('DELETE FROM devices WHERE id = $1', [res.rows[0].id]);

        } catch (e) {
            console.log('❌ Failed:', e.message);
        }

    } catch (err) {
        console.error('Final Error:', err);
    } finally {
        pool.end();
    }
}

debugRegistration();
